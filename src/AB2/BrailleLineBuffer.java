...
public class BrailleLineBuffer implements LineBuffer {
    ...
    private final double growthFactor; // factor by which to increase buffer size when needed

    private final double reductionRatio; // ratio below which the buffer size will be decreased

    private final int minSize;  // size buffer was initialized with, size should never go below this value

    private int count; // running counter of number of characters in the buffer to avoid looping through buffer during every call of count()
    ...
    public void expandBuffer() {
        ...
        int newSize = (int) Math.ceil(growthFactor * size());
    ...
    public void reduceBuffer() {
        // TODO: implementation
        if((double) count() / size() > reductionRatio) return;
        int newSize = (int) Math.ceil(count()*growthFactor);
        if(newSize < minSize || newSize >= size()) return;
        char[][][] newBuffer = new char[newSize][][];
        System.arraycopy(buffer, 0, newBuffer, 0, count());
        buffer = newBuffer;
    }
    ...
    public void push(char[][] bitmap) {
        // TODO: implementation
        expandBuffer();
        buffer[count()] = new char[bitmap.length][bitmap[0].length];
        for(int i = 0; i < bitmap.length; i++) {
            System.arraycopy(bitmap[i], 0, buffer[count()][i], 0, bitmap[i].length);
        }
        count++;
    }
    ...
    public boolean insert(int cursorPosition, char[][] bitmap) {
        ...
        for(int i = count()-1; i >= cursorPosition; i--) {
            buffer[i+1] = buffer[i];
        }
        for(int i = 0; i < bitmap.length; i++) {
            System.arraycopy(bitmap[i], 0, buffer[cursorPosition][i], 0, bitmap[i].length);
        }
        count++;
        return true;
    }
    ...
    public boolean delete(int cursorPosition) {
        // TODO: implementation
        if(cursorPosition < 0 || cursorPosition > count()-1 || count() == 0) return false;
        for (int i = cursorPosition; i < count(); i++) {
            buffer[i] = buffer[i+1];
        }
        count--;
        reduceBuffer();
    ...
    public String[] renderScanlines(int spacing) {
        // TODO: implementation
        if(count() == 0) return null;
        String[] lines = new String[buffer[0].length];
        for(int i = 0; i < buffer[0].length; i++) {
            StringBuilder line = new StringBuilder();
            for(int j = 0; j < count(); j++) {
                line.append(buffer[j][i]);
                if(j+1 < count()) line.append(" ".repeat(spacing));
            }
