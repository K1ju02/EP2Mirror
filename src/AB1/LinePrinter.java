...
public class LinePrinter {
    ...
    // saves character amount allowed in lineBuffer, saved for clearLine()
    ...
    private void createLineBuffer(int lineLength, int cellHeight, int cellWidth){
        ...
        for(int x = 0; x < this.lineBuffer.length; x++){
            for(int y = 0; y < this.lineBuffer[x].length; y++){
                this.lineBuffer[x][y] = ' ';
            }
        }
    }
    ...
    public void clearLine(){
        // TODO: implementation
        createLineBuffer(this.lineLength, this.font.getHeight(), this.font.getWidth());
    }
    ...
    public void printCharacter(char character){
        // TODO: implementation
        char[][] bitmap = this.font.getBitmap(character);
        int currentPos = this.cursorPosition * (this.font.getWidth() + this.spacing);

        if(currentPos + bitmap[0].length > this.lineBuffer[0].length) return;

        for(int x = 0; x < bitmap.length; x++){
            for(int y = 0; y < bitmap[x].length; y++){
                this.lineBuffer[x][currentPos+y] = bitmap[x][y];
            }
        }
        this.cursorPosition++;
    }
